...
void ChthollyTree::divide(int l, int r, int v)
{
    split(l);
    split(r + 1);
    std::set<node>::iterator itl = t.lower_bound(node(l, 0, 0)), itr = t.lower_bound(node(r + 1, 0, 0)), pre = t.end();
    if (itl != t.begin())
        pre = --itl;
    for (auto it = pre == t.end() ? itl : ++itl; it != itr; ++it)
    {
        it->v /= v;
        if (pre != t.end() && it->v == pre->v)
        {
            it->l = pre->l;
            t.erase(pre);
        }
